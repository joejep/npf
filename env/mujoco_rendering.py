...
class CustomOnScreenViewer(BaseRender):
    ...
    def __init__(
        ...
        self.image_viewport = mujoco.MjrRect(10, 10, image_width, image_height)
        self.image_camera = mujoco.MjvCamera()
    ...
    def render_camera_id(self,        
        ...
        if camera_id is not None:
            if camera_id == -1:
                self.image_camera=self.cam
            else:
                self.image_camera.type = mujoco.mjtCamera.mjCAMERA_FIXED
            self.image_camera.fixedcamid = camera_id
        ...
        mujoco.mjr_render(self.image_viewport, self.scn, self.con)
        ...
        rgb_arr_raw = np.zeros(
            3 * self.image_viewport.width * self.image_viewport.height, dtype=np.uint8
        )
        depth_arr = np.zeros(
            self.image_viewport.width * self.image_viewport.height, dtype=np.float32
        )

        mujoco.mjr_readPixels(rgb_arr_raw, depth_arr, self.image_viewport, self.con)

        # Process rendered images according to render_mode
        if render_mode in ["depth_array", "rgbd_tuple"]:
            depth_img = depth_arr.reshape((self.image_viewport.height, self.image_viewport.width))
            # original image is upside-down, so flip it
            depth_img = depth_img[::-1, :]
        if render_mode in ["rgb_array", "rgbd_tuple"]:
            rgb_img = rgb_arr_raw.reshape((self.image_viewport.height, self.image_viewport.width, 3))
            # original image is upside-down, so flip it
            rgb_img = rgb_img[::-1, :]
            ...
            if segmentation:
                ...
                for i in range(self.scn.ngeom):
                    ...
                    if geom.segid != -1:
                        ...
                        seg_ids[geom.segid + 1, 1] = geom.objid
                rgb_img = seg_ids[seg_img]
        return rgb_img,rgb_arr_raw

    def render(self,render_mode=None,camera_id=None):
        """
        ...
        def update():
            ...
            rgb_arr,rgb_arr_raw=self.render_camera_id(camera_id=camera_id)
            depth_arr = np.zeros(
            self.image_viewport.width * self.image_viewport.height, dtype=np.float32
            )
            ...
            mujoco.mjr_render(self.viewport, self.scn, self.con)
            # mujoco.mjr_drawPixels(rgb_arr_raw, depth_arr,
            #         self.image_viewport, self.con)
...
class MujocoRenderer:
    ...
    def _get_viewer(self, render_mode: Optional[str]):
        ...
        if self.viewer is None:
            self.viewer = CustomOnScreenViewer(
                        self.model,
                        self.data,
                        self.width,
                        self.height,
